...
\section{Experimental Protocol and Metrics}
    \text{\Large{This needs to be written better.}}\\
    \text{\Large{Some things need to be added as well, e.g. tiny model, batch size, model length, block size of pretraining etc.}}\\
...
Table~\ref{tab:implemented_schedules} summarizes the noise schedules already provided in the codebase, expressed in terms of the masked probability $p(t)$ and the induced loss scaling.
...
The mean of the second Gaussian distribution is not fixed; rather, it increases progressively throughout the training process:
\begin{equation}
  \mu_2(\tau) = m_s +(m_f-m_s)(1 - \exp(-\tau)),
\end{equation}
where\begin{equation}
 \tau=\tau_{scale}  
  \frac{global\_step}{max\_steps}.
\end{equation}
$m_s$, $m_f$, and $\tau_{\text{scale}}$ are constants. 
$\text{global\_step}$ represents the current training step and 
$\text{max\_steps}$ represents the total number of training steps.

This design enforces the use of a low masking ratio during the initial training stages and a higher masking ratio at later stages, when the model begins to converge.

\subsection{Results}
In all the tables below, we used $\sigma^2=0.1$ for the Gaussian schedules and $\sigma^2_1=0.02, \sigma^2_2=0.08, m_s=0.4, m_f=0.85, \tau_{scale}=3$ for the bimodal Gaussian schedules.
...
We managed to improve the perplexity for the block sizes 128 and 16, but not for block size 4.

We also tested using the bimodal Gaussian schedule, that was best for L=128, during pretraining; Table~\ref{tab:bg_pretraining} reports the best fine-tuning schedule found per pretraining schedule.
...
We observe further improvements for the block sizes 128 and 16, and this time block size 4 improved as well. This indicates that the increasing masking ration due to the increasing $\mu_2$ had an impact.
