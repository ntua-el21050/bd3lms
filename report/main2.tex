...
within each block. We present a reproduction of the original experiments, as well as \hl{our own} ideas for extensions that improve the performance of BD3-LMs.
...
\begin{itemize}
  ...
  \item \textbf{A single \hl{knob} $L'$:} a direct quality--parallelism trade-off.
\end{itemize}
...
To evaluate the efficacy of Block Diffusion models (BD3-LMs) against Autoregressive (AR) and standard Diffusion baselines (SEDD, MDLM), we adopted a \hl{rigorous experimental protocol} tailored to resource-constrained environments.
...
We employed a \hl{comprehensive suite of metrics} to evaluate likelihood, stability, and sample quality:
\begin{itemize}
    \item \textbf{Test Perplexity (PPL):} We report test set perplexity, where lower values indicate better predictive performance.
    \item \textbf{\hl{Objective Stability} (Var.\ NELBO):} To quantify training stability, particularly regarding noise schedules, we measure the Variance of the Negative Evidence Lower Bound \hl{(lower is better)}.
...
We propose \hl{\textbf{We should give credit to their paper}} \cite{kosmopoulou-etal-2025-masked} schedules that directly sample the masked probability $p(t)\in(0,1)$.
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\textwidth}{l Y Y Y Y Y Y}
    ...
    BD3-LM $L'=4$ & 1093.7 & 7857.33 & 1565.6 & \textbf{34.08} & 67332 & 2.4e6 \\
    \bottomrule
  \end{tabularx}
\end{table}

In these runs, sigmoid reweighting yields the best PPL for $L'=4$, far surpassing the base model without reweighting! Several weightings can become unstable (very large perplexities). We notice that the validation loss with reweighting often drops quickly and then starts increasing, so we interpret this phenomenon as overfitting caused by the small dataset and enhanced due to the faster learning of the reweighted objectives.
...
\begin{itemize}
  ...
  \item \textbf{Variance--PPL trade-off:} clipped schedules reduce Var.\ NELBO, \hl{but the best perplexity can come from standard schedules after sufficient optimization.}
...
We also found that (i) bimodal Gaussian schedules can improve PPL/variance in specific settings and (ii) loss reweighting (notably sigmoid weighting in our runs) can yield additional improvements, even though there were unstable results due to overfitting on the small dataset on which we could test.
