...
\author{
  \textbf{Kordas Nikolaos, Kritharidis Konstantinos, Makras Ilias},\\
  \textbf{Markoulidakis Georgios, Ntountounakis Georgios, Vitalis Petros}\\
...
\begin{itemize}
    ...
    \item \textbf{Objective Stability (Var.\ NELBO):} To quantify training stability, particularly regarding noise schedules, we measure the Variance of the Negative Evidence Lower Bound (lower is better).
...
All BD3-LM variants outperform AR on OWT (best: $L'=4$ at 1935 vs.\ AR's 2036), inverting the paper's ordering where AR (17.54) leads.
We attribute this to faster convergence of block diffusion at small training budgets; with more compute, AR would likely close this gap.
SEDD and MDLM lag behind AR, consistent with the paper.
The BD3-LM variants are tightly clustered within 6 PPL points, suggesting reduced block-size sensitivity on OWT's longer documents.
...
The paper evaluates on seven benchmarks (PTB, Wikitext, LM1B, Lambada, AG~News, Pubmed, Arxiv).
Due to computational constraints and dataset availability within the codebase's data loaders, we focused on three representative datasets: LM1B and Lambada---which differ substantially from OWT in domain and style---and Wikitext, which is structurally similar to OWT as web/encyclopedic text.
...
BD3-LM's advantage over AR is domain-dependent: under domain shift (LM1B, Lambada), BD3-LM outperforms AR by $\sim$8\%, but on similar-domain Wikitext, AR retains a $\sim$8.6\% advantage.

This asymmetry admits a natural interpretation.
AR models are trained to predict the exact next token given a left context, a strategy that is maximally rewarded when test sequences follow distributions close to the training data.
When the domain shifts, memorized token co-occurrence patterns become less useful and may even hurt generalization.
BD3-LM's denoising objective, by contrast, requires the model to reconstruct masked tokens from \emph{bidirectional} context within each block, forcing the model to learn more robust, compositional representations that capture structural regularities rather than surface-level distributional statistics.

LM1B's short sentences align naturally with block boundaries, and Lambada's long-range prediction benefits from bidirectional block attention.
Wikitext overlaps with OWT in register and topic, so AR's distributional specialization pays off.

BD3-LM also consistently outperforms SEDD and MDLM, indicating that block-autoregressive structure adds generalization beyond denoising alone.
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\textwidth}{l Y Y Y Y Y Y}
    ...
    BD3-LM $L'=8$ & 1210 & 7954.17 & 1569.75 & \textbf{1078.57} & 109169 & 3.6e10 \\
    BD3-LM $L'=4$ & 1176 & 7857.33 & 1565.6 & 1093.7 & 67332 & 2.4e6 \\
    \bottomrule
  \end{tabularx}
\end{table}
